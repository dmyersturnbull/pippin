...
class IfElseGrammar(val input: ParserInput, tolerance: Double = 0.000001, seed: Option[Int] = None, functions: Map[String, Seq[Double] => Double] = RealNumberGrammar.defaultFunctionMap) extends Parser {
	...
	def ifElif: Rule1[Option[Double]] = rule {
		"if" ~ conditionRule ~ elif ~> ((a: Option[Double], b: Option[Double]) => if (a.isDefined) a else if (b.isDefined) b else None)
	}

	def elif: Rule1[Option[Double]] = rule {
